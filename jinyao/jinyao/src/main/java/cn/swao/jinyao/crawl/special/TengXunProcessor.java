...
public class TengXunProcessor implements PageProcessor {
    ...
    public static void main(String[] args) {
        Spider spider = Spider.create(new TengXunProcessor()).addPipeline(new JsonFilePipeline(path)).addPipeline(new ConsolePipeline()).thread(THREAD_NUM);
    ...
    public void process(Page page) {
        if (page.getUrl().regex(JSON_URL_REGEX).match()) {
            ...
            try {
                ...
                page.putField("content", news.getContentElement().html());
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private Site site = Site.me().addHeader("Referer", REFERER).setUserAgent("Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36").setSleepTime(1000).setTimeOut(1000 * 10).setRetryTimes(10).setCycleRetryTimes(10);
}
